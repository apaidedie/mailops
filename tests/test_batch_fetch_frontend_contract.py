...
class BatchFetchFrontendContractTests(unittest.TestCase):
    ...
    def test_selected_account_ids_semantics_unchanged(self):
        ...
        # Selected-count label translates via pattern, not getUiLanguage ternary.
        self.assertIn("function formatSelectedItemsLabel(count)", js)
        fn_start = js.index("function formatSelectedItemsLabel(count)")
        fn_end = js.index("const pickApiMessage", fn_start)
        fn_slice = js[fn_start:fn_end]
        self.assertIn("translateAppTextLocal('已选 ' + count + ' 项')", fn_slice)
        self.assertNotIn("getUiLanguage() === 'en'", fn_slice)
