...
from outlook_web.logging_config import JsonLogFormatter, configure_runtime_logging


class LoggingConfigTests(unittest.TestCase):
    def setUp(self):
        self.namespace_logger = logging.getLogger("outlook_web")
    ...
    def test_json_formatter_emits_stable_fields_and_safe_extras(self):
        ...
        self.assertEqual(payload["logger"], "outlook_web.tests")
    ...
    def test_json_formatter_adds_request_context_without_query_string(self):
        ...
        with app.test_request_context("/api/mailboxes?token=hidden", method="POST", environ_base={"REMOTE_ADDR": "127.0.0.1"}):
            ...
            record = logging.LogRecord("outlook_web.request", logging.INFO, __file__, 80, "request complete", (), None)
    ...
    def test_runtime_configuration_replaces_only_managed_handler(self):
        app = Flask("outlook_web.logging-test")
        ...
        managed = [handler for handler in self.namespace_logger.handlers if getattr(handler, "_outlook_web_managed", False)]
    ...
    def test_default_text_formatter_keeps_readable_legacy_shape(self):
        app = Flask("outlook_web.text-logging-test")
        stream = io.StringIO()
        ...
        self.assertRegex(line, r"^\d{2}:\d{2}:\d{2} outlook_web\.text-logging-test INFO readable line$")
