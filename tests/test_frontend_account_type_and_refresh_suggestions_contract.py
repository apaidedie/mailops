...
class FrontendAccountTypeContractTests(unittest.TestCase):
    ...
    def test_account_list_provider_labels_are_catalog_driven(self):
        ...
        self.assertIn("resolveMailboxProviderLabel", groups_js)
        self.assertIn("function resolveMailboxProviderLabel", main_js)
        self.assertIn("function getMailboxProviderCatalogLabel", main_js)
        self.assertIn("account-provider-tag", groups_js)
        self.assertNotIn("cloudflare_temp_mail: 'CF 临时邮箱'", groups_js)
        self.assertNotIn("qq: 'QQ 邮箱'", groups_js)
        self.assertNotIn("custom: '自定义 IMAP'", groups_js)
...
class RefreshErrorSuggestionsBehaviorNodeTests(unittest.TestCase):
    def test_build_refresh_error_suggestions_branches_by_account_type_provider_and_error(
        ...
        core_dir = repo_root / "static" / "js" / "core"
...
const coreDir = process.argv.slice(1).find((arg) => arg && !arg.startsWith('-') && /core$/.test(arg.replace(/\\/g, '/')));
if (!coreDir) {
  throw new Error('missing core js path: ' + JSON.stringify(process.argv));
}
...
"""
        ...
        result = subprocess.run(
            ["node", "-e", node_script, str(core_dir)],
            capture_output=True,
            text=True,
            check=False,
        )
