...
class OAuthToolApiBlueprintTests(OAuthToolTestBase):
    ...
    def test_token_tool_js_soft_loads_config_and_accounts(self):
        """token_tool.js soft-loads warm config/accounts and coalesces concurrent loads."""
        from pathlib import Path

        js_path = Path(__file__).resolve().parents[1] / "static" / "js" / "features" / "token_tool.js"
        js_text = js_path.read_text(encoding="utf-8")

        self.assertIn("let oauthConfigCache = null", js_text)
        self.assertIn("let oauthConfigLoadPromise = null", js_text)
        self.assertIn("let oauthConfigLoadForce = false", js_text)
        self.assertIn("let tokenToolAccountsCache = null", js_text)
        self.assertIn("let tokenToolAccountsLoadPromise = null", js_text)
        self.assertIn("let tokenToolAccountsLoadForce = false", js_text)
        self.assertIn("async function loadOAuthConfig(forceRefresh = false)", js_text)
        self.assertIn("async function loadAccountOptions(forceRefresh = false)", js_text)
        self.assertIn("function invalidateTokenToolAccountsCache", js_text)
        self.assertIn("function applyOAuthConfig", js_text)
        self.assertIn("function applyTokenToolAccountOptions", js_text)

        config_start = js_text.index("async function loadOAuthConfig(forceRefresh = false)")
        config_end = js_text.index("async function startOAuth", config_start)
        config_slice = js_text[config_start:config_end]
        self.assertIn("!force && oauthConfigCache", config_slice)
        self.assertIn("if (!force || oauthConfigLoadForce)", config_slice)
        self.assertIn("oauthConfigLoadForce = force", config_slice)
        self.assertIn("oauthConfigLoadPromise !== request", config_slice)
        self.assertIn("oauthConfigLoadPromise = request", config_slice)
        # Soft re-entry must not clobber in-progress form edits.
        self.assertIn("function isOAuthConfigFormUnhydrated()", js_text)
        self.assertIn("isOAuthConfigFormUnhydrated()", config_slice)
        self.assertIn("if (force || isOAuthConfigFormUnhydrated())", config_slice)

        accounts_start = js_text.index("async function loadAccountOptions(forceRefresh = false)")
        accounts_end = js_text.index("async function openSaveDialog", accounts_start)
        accounts_slice = js_text[accounts_start:accounts_end]
        self.assertIn("!force && Array.isArray(tokenToolAccountsCache)", accounts_slice)
        self.assertIn("if (!force || tokenToolAccountsLoadForce)", accounts_slice)
        self.assertIn("tokenToolAccountsLoadForce = force", accounts_slice)
        self.assertIn("tokenToolAccountsLoadPromise !== request", accounts_slice)
        self.assertIn("tokenToolAccountsLoadPromise = request", accounts_slice)
        # Paint account select only while save dialog is open.
        self.assertIn("function isTokenToolSaveDialogOpen()", js_text)
        self.assertIn("isTokenToolSaveDialogOpen()", accounts_slice)
        self.assertIn("// Always warm soft cache; paint only while save dialog is open.", accounts_slice)
        apply_start = js_text.index("function applyTokenToolAccountOptions(accounts)")
        apply_end = js_text.index("function invalidateTokenToolAccountsCache", apply_start)
        self.assertIn("if (!isTokenToolSaveDialogOpen())", js_text[apply_start:apply_end])
        inv_start = js_text.index("function invalidateTokenToolAccountsCache")
        inv_end = js_text.index("async function loadAccountOptions", inv_start)
        self.assertIn("tokenToolAccountsLoadForce = false", js_text[inv_start:inv_end])

        open_start = js_text.index("async function openSaveDialog")
        open_end = js_text.index("function closeSaveDialog", open_start)
        self.assertIn("await loadAccountOptions(false)", js_text[open_start:open_end])

        save_start = js_text.index("async function confirmSaveToAccount")
        save_end = js_text.index("document.addEventListener('DOMContentLoaded'", save_start)
        save_slice = js_text[save_start:save_end]
        self.assertIn("invalidateTokenToolAccountsCache()", save_slice)
        # Token write must drop soft mail caches for the target mailbox.
        self.assertIn("clearEmailListCacheForMailbox(targetEmail)", save_slice)
        self.assertIn("invalidateAccountsCache()", save_slice)
        self.assertIn("invalidateUnifiedMailboxDirectoryCache()", save_slice)

        # Boot soft-loads config; saveConfig updates oauthConfigCache.
        boot_start = js_text.index("document.addEventListener('DOMContentLoaded'")
        self.assertIn("loadOAuthConfig(false)", js_text[boot_start:])
        save_cfg_start = js_text.index("async function saveConfig")
        save_cfg_end = js_text.index("function copyResultField", save_cfg_start)
        self.assertIn("oauthConfigCache =", js_text[save_cfg_start:save_cfg_end])

        # Language change soft-paints warm chrome without force network reloads.
        self.assertIn("window.addEventListener('ui-language-changed'", js_text)
        lang_start = js_text.index("window.addEventListener('ui-language-changed'")
        lang_end = js_text.index("document.addEventListener('DOMContentLoaded'", lang_start)
        lang_slice = js_text[lang_start:lang_end]
        self.assertIn("renderScopeChips(scopeValue)", lang_slice)
        self.assertIn("applyTokenToolAccountOptions(tokenToolAccountsCache)", lang_slice)
        self.assertNotIn("loadOAuthConfig(true)", lang_slice)
        self.assertNotIn("loadAccountOptions(true)", lang_slice)
        # Live translate helper so language switches re-translate labels.
        self.assertIn("function t(text)", js_text)
        self.assertIn("window.translateAppText(text)", js_text)
