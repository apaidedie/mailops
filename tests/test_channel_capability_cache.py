...
实现时需新建 mailops/services/channel_capability_cache.py。
"""
...
class TestChannelCapabilityCache(unittest.TestCase):
    ...
    def setUp(self):
        """每个测试前清空缓存"""
        try:
            from mailops.services.channel_capability_cache import clear_all
    ...
    def test_set_available_and_get(self):
        from mailops.services.channel_capability_cache import get_status, set_status
    ...
    def test_unavailable_channel_filtered_in_plan(self):
        from mailops.services.channel_capability_cache import (
            filter_channel_plan,
            set_status,
        )
    ...
    def test_no_cache_returns_none(self):
        from mailops.services.channel_capability_cache import get_status

        status = get_status("new@test.com", "imap_new")
        self.assertIsNone(status)

    # C-05: TTL 过期 → 返回 None
    @patch("mailops.services.channel_capability_cache.time.monotonic")
    ...
    def test_filter_no_cache_returns_all(self):
        from mailops.services.channel_capability_cache import filter_channel_plan
