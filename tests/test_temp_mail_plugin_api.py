...
class TestPluginAPI(unittest.TestCase):
    ...
    def test_get_plugins_includes_contract_validation_summary_for_loaded_plugin(self):
        ...
        data = resp.get_json()
        result = data.get("data", data)
        plugin = next(item for item in result.get("plugins", []) if item.get("name") == "mock_api")

        self.assertEqual(resp.status_code, 200)
        validation = plugin.get("contract_validation") or {}
        self.assertEqual(validation.get("provider"), "mock_api")
        self.assertIn(validation.get("status"), {"valid", "warning", "invalid"})
        self.assertIn("errors", validation.get("summary") or {})
    ...
    def test_get_plugin_contract_success(self):
        ...
        class ContractApiProvider(TempMailProviderBase):
            provider_name = "contract_api"
            provider_label = "Contract API"
            provider_version = "1.0.0"
            config_schema = {"fields": [{"key": "url", "label": "URL", "type": "url"}]}
        ...
        try:
            resp = self._client.get("/api/plugins/contract_api/contract")
            data = resp.get_json()
            result = data.get("data", data)
            self.assertEqual(resp.status_code, 200)
            self.assertEqual(result.get("plugin_name"), "contract_api")
            validation = result.get("contract_validation") or {}
            self.assertEqual(validation.get("provider"), "contract_api")
            self.assertEqual(validation.get("status"), "valid")
            self.assertNotIn("sk-", str(result))
        finally:
            self._registry.pop("contract_api", None)
    ...
    def test_get_plugin_contract_marks_non_base_provider_invalid(self):
        ...
        class LooseContractProvider:
            provider_name = "loose_contract"
            provider_label = "Loose Contract"
            provider_version = "1.0.0"
        ...
        try:
            resp = self._client.get("/api/plugins/loose_contract/contract")
            data = resp.get_json()
            result = data.get("data", data)
            validation = result.get("contract_validation") or {}

            self.assertEqual(resp.status_code, 200)
            self.assertEqual(validation.get("provider"), "loose_contract")
            self.assertEqual(validation.get("status"), "invalid")
            self.assertIn(
                "PROVIDER_BASE_CLASS_INVALID",
                {issue.get("code") for issue in validation.get("issues") or []},
            )
        finally:
            self._registry.pop("loose_contract", None)

    def test_get_plugin_contract_not_loaded(self):
        """未加载插件的契约端点返回 PLUGIN_NOT_LOADED"""
        resp = self._client.get("/api/plugins/not_loaded_contract/contract")
        data = resp.get_json()
        self.assertEqual(resp.status_code, 404)
        self.assertEqual(data.get("code"), "PLUGIN_NOT_LOADED")
