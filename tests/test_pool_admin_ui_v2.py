...
class PoolAdminJsV2FunctionTests(PoolAdminUIv2Base):
    ...
    def test_declares_catalog_driven_provider_options(self):
        """类型筛选选项应由 catalog 动态填充。"""
        self.assertIn("function ensurePoolAdminProviderOptions", self.js)
        self.assertIn("function applyPoolAdminProviderOptions", self.js)
        self.assertIn("function normalizePoolAdminProviderOptions", self.js)
        self.assertIn("ensurePoolAdminProviderOptions()", self.js)
        self.assertIn("/api/providers", self.js)
        self.assertIn("mailboxProviderCatalogCache", self.js)
...
class PoolAdminDomStructureTests(PoolAdminUIv2Base):
    ...
    def test_provider_filter_has_no_hardcoded_provider_options(self):
        """类型筛选模板应只保留空占位，不硬编码 provider 列表。"""
        select_start = self.html.index('id="poolAdminProviderFilter"')
        ...
        for provider in ("outlook", "imap", "custom", "cloudflare_temp_mail"):
