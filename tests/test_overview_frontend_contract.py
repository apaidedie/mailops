...
class OverviewFrontendContractTests(unittest.TestCase):
    ...
    def test_summary_command_center_render_contract(self):
        ...
        self.assertLess(summary_slice.index('renderOverviewCommandCenter(data.command_center || {})'), summary_slice.index('<div class="kpi-row">'))
    ...
    def test_invalid_token_governance_soft_loads_warm_cache(self):
        ...
        modal_end = js_text.index('async function autoLoadFailedListIfNeeded', modal_start) if 'async function autoLoadFailedListIfNeeded' in js_text[modal_start:modal_start+800] else js_text.index('// Soft-load cache for failed refresh logs', modal_start)
    ...
    def test_refresh_modal_history_soft_loads_warm_cache(self):
        ...
        failed_end = js_text.index('function renderRefreshModalHistory', failed_start) if 'function renderRefreshModalHistory' in js_text[failed_start:failed_start+50] else js_text.index('let refreshModalHistoryCache', failed_start)
    ...
    def test_failed_refresh_list_soft_loads_and_coalesces(self):
        ...
        raw_hits = [
            ...
            if "fetch('/api/accounts/refresh-logs/failed')" in line
        ]
    ...
    def test_load_accounts_by_group_coalesces_inflight(self):
        ...
        self.assertIn('async function loadAccountsByGroup(groupId, forceRefresh = false, page = currentAccountPage)', groups_js)
        load_start = groups_js.index('async function loadAccountsByGroup(groupId, forceRefresh = false, page = currentAccountPage)')
        load_end = groups_js.index('const accountListMetaCache', load_start) if 'const accountListMetaCache' in groups_js[load_start:load_start+50] else groups_js.index('// 排序相关变量', load_start)
        ...
        self.assertIn('!force && Array.isArray(accountsCache[groupId]) && cachedMeta && cachedMeta.queryKey === queryKey', load_slice)
    ...
    def test_batch_move_groups_soft_loads_warm_groups(self):
        ...
        soft_lang = next(
            (main_js[idx:idx + 5000]
             for idx in [i for i in range(len(main_js)) if main_js.startswith("window.addEventListener('ui-language-changed'", i)]
    ...
    def test_load_deployment_info_soft_loads_warm_cache(self):
        ...
        self.assertIn('lastDeploymentInfo && (typeof isSettingsSurfaceActive === \'function\' ? isSettingsSurfaceActive() : true)', js_text)
    ...
    def test_load_temp_email_options_soft_loads_and_force_supersedes(self):
        ...
        self.assertLess(list_slice.index('invalidateTempEmailOptionsCache()'), list_slice.index('syncTempEmailProviderSelection'))
    ...
    def test_get_temp_email_detail_soft_loads_and_coalesces(self):
        ...
        self.assertIn('clearTempEmailDetailCacheForMailbox(key)', temp_js[
            temp_js.index('function clearTempEmailMessagesCacheForMailbox'):
            temp_js.index('function invalidateTempEmailDetailCacheEntry')
        ])
    ...
    def test_select_email_detail_soft_loads_and_coalesces(self):
        ...
        temp_delete_end = emails_js.index('// 选择邮件', temp_delete_start) if '// 选择邮件' in emails_js[temp_delete_start:temp_delete_start + 2000] else emails_js.index('async function selectEmail', temp_delete_start)
    ...
    def test_account_delete_clears_email_list_soft_cache(self):
        ...
        del_current = accounts_js[
            accounts_js.index('async function deleteCurrentAccount()'):
            accounts_js.index('async function toggleAccountStatus')
        ]
        ...
        del_quick = accounts_js[
            accounts_js.index('async function deleteAccount(accountId, email)'):
            accounts_js.index('async function batchNotificationToggle')
        ]
        ...
        select_end = accounts_js.index('async function showEditAccountModal', select_start) if 'async function showEditAccountModal' in accounts_js[select_start:select_start+5000] else accounts_js.index('function invalidateAccountDetailCache', select_start)
        ...
        update_slice = accounts_js[
            accounts_js.index('async function updateAccount()'):
            accounts_js.index('async function deleteCurrentAccount()')
        ]
        ...
        batch = main_js[
            main_js.index('async function batchDeleteAccounts()'):
            main_js.index('function resolveSelectedAccountsForBatchFetch')
        ]
        ...
        add_end = accounts_js.index('// 显示编辑', add_start) if '// 显示编辑' in accounts_js[add_start:add_start + 8000] else accounts_js.index('async function showEditAccountModal', add_start)
    ...
    def test_navigate_soft_loads_settings_page(self):
        ...
        raw_get_hits = [
            line for line in js_text.splitlines()
            if "fetch('/api/settings')" in line and 'method' not in line
        ]
    ...
    def test_navigate_soft_loads_refresh_and_audit_log_pages(self):
        ...
        refresh_all = js_text[js_text.index('async function refreshAllAccounts()'):js_text.index('async function retryFailedAccounts')]
        self.assertIn(
            "translateAppTextLocal(\n                                    '刷新完成！成功: ' + data.success_count + ', 失败: ' + data.failed_count\n                                )",
            refresh_all,
        )
        self.assertNotIn("getUiLanguage() === 'en'\n                                    ? `Refresh completed", refresh_all)
        retry_failed = js_text[js_text.index('async function retryFailedAccounts'):js_text.index('async function retrySingleAccount')]
        self.assertIn(
            "translateAppTextLocal(\n                                '重试完成！成功: ' + data.success_count + ', 失败: ' + data.failed_count\n                            )",
            retry_failed,
        )
        retry_single = js_text[js_text.index('async function retrySingleAccount'):js_text.index('function showFailedListFromData')]
    ...
    def test_overview_i18n_has_current_labels(self):
        ...
        self.assertIn("'\u805a\u5408\u90ae\u7bb1\u3001Provider \u4e0e\u5916\u90e8 API \u63a5\u5165\u72b6\u6001': 'Mailbox, provider, and external API readiness'", i18n_text)
