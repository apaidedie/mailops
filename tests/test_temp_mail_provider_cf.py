    ...
    def test_cf_worker_config_can_come_from_environment(self):
        with self.app.app_context():
            ...
            with patch.dict(
                "os.environ",
                {
                    "CF_WORKER_BASE_URL": "https://env-worker.example.workers.dev/",
                    "CF_WORKER_ADMIN_KEY": "env-admin-key",
                },
            ):
                provider = self._make_provider()

                self.assertEqual(provider._base_url(), "https://env-worker.example.workers.dev")
                self.assertEqual(provider._admin_key(), "env-admin-key")
