...
class TestGPTMailErrorHandling(unittest.TestCase):
    ...
    def test_normalize_temp_mail_api_base_url_accepts_docs_page_url(self):
        ...
        self.assertEqual(
            normalize_temp_mail_api_base_url("https://custom.example/base/api/"),
            "https://custom.example/base",
        )
        self.assertEqual(
            normalize_temp_mail_api_base_url("https://custom.example/base/api/v1"),
            "https://custom.example/base/api/v1",
        )

    def test_gptmail_request_normalizes_docs_page_base_url_before_calling_api(self):
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"success": True, "data": {"email": "demo@example.com"}}

        with patch("mailops.services.gptmail.get_temp_mail_api_base_url", return_value="https://mail.chatgpt.org.uk/zh/api"):
            with patch("mailops.services.gptmail.get_gptmail_api_key", return_value="test_key"):
        ...
        self.assertEqual(get_mock.call_args.args[0], "https://mail.chatgpt.org.uk/api/generate-email")

    def test_gptmail_request_no_api_key(self):
        """测试 API Key 未配置的情况"""
        with patch("mailops.services.gptmail.get_gptmail_api_key", return_value=""):
    ...
    def test_gptmail_request_401_unauthorized(self):
        ...
        with patch(
            "mailops.services.gptmail.get_gptmail_api_key",
    ...
    def test_generate_temp_email_success(self):
        ...
        with patch("mailops.services.gptmail.gptmail_request", return_value=mock_result):
    ...
    def test_generate_temp_email_with_prefix_and_domain(self):
        ...
        with patch("mailops.services.gptmail.gptmail_request", return_value=mock_result) as mock_request:
