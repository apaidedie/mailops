...
class JSContractTests(unittest.TestCase):
    ...
    def test_check_version_update_function(self):
        """checkVersionUpdate 函数存在（支持 forceRefresh soft-load）"""
        js = self._get_js()
        self.assertIn("async function checkVersionUpdate(forceRefresh = false)", js)
        self.assertIn("versionCheckCache", js)
        self.assertIn("versionCheckLoadPromise", js)
    ...
    def test_uses_current_version_not_current(self):
        ...
        js = self._get_js()
        # 精确定位 apply + checkVersionUpdate 函数体（banner 字段在 apply helper）
        idx = js.index("function applyVersionCheckPayload(data)")
        ...
        # Banner chrome is translated at paint time for language soft re-paint.
        self.assertIn("translateAppTextLocal('发现新版本')", block)
        self.assertIn("translateAppTextLocal('查看更新日志')", block)
        # Soft-load session cache + force supersedes soft in-flight.
        self.assertIn("!force && versionCheckCache", block)
        self.assertIn("versionCheckLoadPromise", block)
        self.assertIn("if (!force || versionCheckLoadForce)", block)
        self.assertIn("versionCheckLoadForce = force", block)
        self.assertIn("fetch('/api/system/version-check')", block)
