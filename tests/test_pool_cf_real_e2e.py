...
依赖环境:
- RUN_REAL_CF_E2E=true
- REAL_CF_WORKER_BASE_URL 已配置且真实可访问
- REAL_CF_WORKER_ADMIN_KEY 已配置
- REAL_CF_WORKER_DOMAIN 已配置
...
_REAL_CF_E2E_ENV_KEYS = (
    ...
    "REAL_CF_WORKER_ADMIN_KEY",
    "REAL_CF_WORKER_DOMAIN",
)


def _real_cf_e2e_enabled() -> bool:
    if os.environ.get("RUN_REAL_CF_E2E", "").strip().lower() != "true":
        return False
    return all(os.environ.get(key, "").strip() for key in _REAL_CF_E2E_ENV_KEYS[1:])


@unittest.skipUnless(
    _real_cf_e2e_enabled(),
    "Set RUN_REAL_CF_E2E=true plus REAL_CF_WORKER_BASE_URL, REAL_CF_WORKER_ADMIN_KEY, and REAL_CF_WORKER_DOMAIN to run real CF Worker E2E tests",
)
class RealCFWorkerE2ETests(unittest.TestCase):
    """真实 CF Worker API 端到端测试"""

    CF_BASE_URL = os.environ.get("REAL_CF_WORKER_BASE_URL", "").strip()
    CF_ADMIN_KEY = os.environ.get("REAL_CF_WORKER_ADMIN_KEY", "").strip()
    CF_DOMAIN = os.environ.get("REAL_CF_WORKER_DOMAIN", "").strip()
