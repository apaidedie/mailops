...
class ExternalApiTempMailCompatTests(unittest.TestCase):
    ...
    def test_task_mailbox_external_read_matrix_covers_list_latest_detail_and_verification_endpoints(self):
        ...
        with patch(
            ...
            detail_resp = client.get(
                f"/api/v1/external/messages/{message_id}?email={email_addr}",
                headers=self._headers("compat-key"),
            )
    ...
    def test_finish_rejects_external_read_matrix_for_task_mailbox(self):
        ...
        finish_resp = client.post(
            "/api/v1/external/temp-emails/tmptask_finish_matrix/finish",
            headers=self._headers("compat-key"),
    ...
    def test_account_status_returns_temp_mailbox_status_contract(self):
        email_addr = self._create_task_mailbox(email_addr="status@compat-temp.test", task_token="tmptask_status")
        client = self.app.test_client()
        ...
        self.assertEqual(data["provider_name"], "custom_domain_temp_mail")
        self.assertEqual(data["read_capability"], "temp_provider")
        self.assertEqual(
            data["external_mailbox_read_contract"]["next_actions"]["read_latest_message"]["endpoint"],
            "/api/v1/external/messages/latest",
        )

        finish_resp = client.post(
            "/api/v1/external/temp-emails/tmptask_status/finish",
            headers=self._headers("compat-key"),
            json={"result": "success"},
        )
        self.assertEqual(finish_resp.status_code, 200)
        ...
        self.assertEqual(finished_resp.status_code, 200)
        finished_data = finished_resp.get_json()["data"]
        self.assertEqual(finished_data["status"], "finished")
        self.assertFalse(finished_data["can_read"])
    ...
    def test_other_consumer_gets_403_for_task_mailbox_read_wait_and_probe_paths(self):
        ...
        wait_owner_resp = client.get(
            f"/api/v1/external/wait-message?mode=async&email={email_addr}&timeout_seconds=30&poll_interval=5",
    ...
    def test_finish_transitions_pending_probe_to_probe_cancelled_contract(self):
        ...
        finish_resp = client.post(
            "/api/v1/external/temp-emails/tmptask_probe/finish",
            headers=self._headers("compat-key"),
            json={"result": "success", "detail": "done"},
        )
        self.assertEqual(finish_resp.status_code, 200)
