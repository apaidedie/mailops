...
def _int_body_field(
    body: dict[str, Any], name: str, default: int, *, minimum: int | None = None, maximum: int | None = None
) -> int:
...
def _read_session_action(
    *,
    ...
    if read_action in {"message_detail", "message_raw"}:
        ...
        if read_action == "message_raw":
            ...
            if feature_resp is not None:
                raise external_api_service.FeatureDisabledError(
                    "功能 raw_content 在公网模式下已禁用", data={"feature": "raw_content"}
                )
    ...
    if feature_resp is not None:
        raise external_api_service.FeatureDisabledError(
            "功能 wait_message 在公网模式下已禁用", data={"feature": "wait_message"}
        )
