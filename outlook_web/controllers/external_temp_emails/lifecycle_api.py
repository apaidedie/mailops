...
def api_external_apply_temp_email():
    ...
    try:
        ...
        _audit(
            endpoint,
            "ok",
            details={"task_token": mailbox["task_token"], "domain": mailbox["domain"], "provider_name": payload["provider_name"]},
...
def api_external_finish_temp_email(task_token: str):
    ...
    if len(result_text) > DETAIL_MAX_LEN or len(detail_text) > DETAIL_MAX_LEN:
        _audit(endpoint, "error", details={"code": "INVALID_PARAM", "task_token": task_token}, email_addr=str(mailbox.get("email") or ""))
