...
_SCAFFOLD_PROVIDER_KEY_PATTERN = re.compile(r"^[A-Za-z0-9][A-Za-z0-9_-]*$")
_TEMPLATE_PROVIDER_KEY = "template_temp_mail"
_TEMPLATE_PROVIDER_CLASS = "TemplateTempMailProvider"
_TEMPLATE_PROVIDER_LABEL = "Template Temp Mail"
...
def _get_provider_template_file() -> Path:
    return Path(__file__).resolve().parents[2] / "examples" / "temp_mail_provider_plugin_template.py"


def _normalize_scaffold_provider_key(provider_key: str) -> str:
    value = str(provider_key or "").strip()
    if not value or not _SCAFFOLD_PROVIDER_KEY_PATTERN.match(value):
        raise PluginManagerError(
            "PLUGIN_PROVIDER_KEY_INVALID",
            "Provider key must be stable ASCII and may contain only letters, numbers, underscores, or hyphens.",
            status=400,
...
def _provider_key_parts(provider_key: str) -> list[str]:
    return [part for part in re.split(r"[^A-Za-z0-9]+", provider_key) if part]


def _provider_class_name(provider_key: str) -> str:
    words = _provider_key_parts(provider_key)
    class_root = "".join(part[:1].upper() + part[1:] for part in words) or "Provider"
    if class_root[0].isdigit():
        class_root = f"Provider{class_root}"
    suffix = "Provider" if class_root.endswith("TempMail") else "TempMailProvider"
    return f"{class_root}{suffix}"


def _provider_label(provider_key: str) -> str:
    words = _provider_key_parts(provider_key)
    return " ".join(part[:1].upper() + part[1:] for part in words) or provider_key
...
def scaffold_provider_plugin(
    ...
    *,
    ...
    plugin_name = _normalize_scaffold_provider_key(provider_key)
    template_file = _get_provider_template_file()
    if not template_file.is_file():
        raise PluginManagerError(
            "PLUGIN_TEMPLATE_NOT_FOUND",
            f"Provider template not found: {template_file}",
            status=500,
            data={"template_path": str(template_file)},
        )

    target_dir = Path(output_dir).expanduser().resolve() if output_dir is not None else _ensure_plugin_dir()
    target_dir.mkdir(parents=True, exist_ok=True)
    target_file = target_dir / f"{plugin_name}.py"
    if target_file.exists() and not force:
        raise PluginManagerError(
            "PLUGIN_SCAFFOLD_EXISTS",
            f"Provider plugin already exists: {target_file}",
            status=409,
            data={"file_path": str(target_file), "provider_key": plugin_name},
        )

    class_name = _provider_class_name(plugin_name)
    label = _provider_label(plugin_name)
    content = template_file.read_text(encoding="utf-8")
    content = content.replace(f'PROVIDER_KEY = "{_TEMPLATE_PROVIDER_KEY}"', f'PROVIDER_KEY = "{plugin_name}"')
    content = content.replace(_TEMPLATE_PROVIDER_CLASS, class_name)
    content = content.replace(f'provider_label = "{_TEMPLATE_PROVIDER_LABEL}"', f'provider_label = "{label}"')
    content = content.replace("Template provider API key is not configured", f"{label} API key is not configured")
...
def get_plugin_contract_validation(name: str, *, probe_options: bool = True) -> dict[str, Any]:
    plugin_name = str(name or "").strip()
    from outlook_web.temp_mail_registry import _REGISTRY
    ...
    return {
        "plugin_name": plugin_name,
        "contract_validation": validate_temp_mail_provider_class(plugin_name, provider_cls, probe_options=probe_options),
    }
