...
class TempMailService:
    ...
    def _get_provider(
        ...
        *,
        ...
        if resolved_provider_name and not is_mailbox_provider_active("temp", resolved_provider_name):
            raise TempMailError(
                "MAILBOX_PROVIDER_NOT_ACTIVE",
                f"临时邮箱 Provider 未启用: {resolved_provider_name}",
                status=400,
    ...
    def apply_task_mailbox(
        ...
        *,
        ...
        if len(normalized_caller_id) > CALLER_ID_MAX_LEN:
            raise TempMailError("INVALID_PARAM", f"caller_id 超过最大长度 {CALLER_ID_MAX_LEN}", status=400)
        if not normalized_task_id:
            raise TempMailError("INVALID_PARAM", "task_id 必填", status=400)
        if len(normalized_task_id) > TASK_ID_MAX_LEN:
            raise TempMailError("INVALID_PARAM", f"task_id 超过最大长度 {TASK_ID_MAX_LEN}", status=400)
        normalized_prefix_hint = str(prefix or "").strip()
        normalized_domain_hint = str(domain or "").strip()
        if normalized_prefix_hint and len(normalized_prefix_hint) > TASK_MAILBOX_PREFIX_MAX_LEN:
            raise TempMailError("PREFIX_INVALID", "前缀长度不符合要求", status=400)
        if normalized_domain_hint and len(normalized_domain_hint) > EMAIL_DOMAIN_MAX_LEN:
            raise TempMailError("DOMAIN_INVALID", "域名长度不符合要求", status=400)
        normalized_pn = str(provider_name or "").strip() or None
        normalized_prefix, normalized_domain = self._validate_prefix_and_domain(prefix, domain, provider_name=normalized_pn)
        provider = self._get_provider(provider_name=normalized_pn, purpose="runtime")
        result = self._create_mailbox(provider, prefix=normalized_prefix, domain=normalized_domain)
        ...
        email_addr = str(result.get("email") or "").strip()
        if not email_addr:
            raise TempMailError("TEMP_EMAIL_CREATE_FAILED", "任务邮箱创建失败：缺少邮箱地址", status=502)
        ...
        mailbox_provider_name = str(
            result.get("provider_name")
            or result_meta.get("provider_name")
            or getattr(provider, "provider_name", "")
            or mailbox.get("provider_name")
            or normalized_pn
            or ""
        ).strip()
        mailbox["provider_name"] = mailbox_provider_name
        mailbox["provider_label"] = temp_mail_provider_label(mailbox_provider_name)
        catalog_item = get_provider_catalog_item("temp", mailbox_provider_name)
        mailbox["read_capability"] = str((catalog_item or {}).get("read_capability") or "temp_provider")
        return mailbox
