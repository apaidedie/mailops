...
当前项目已经完成临时邮箱 Provider 插件化。对接一个新的 Provider，不需要再改 Route / Controller / Service 主干，也不需要改现有 `/api/temp-emails/*` 或 `/api/v1/external/temp-emails/*` 路径。
...
6. 点击“应用变更”或调用 `POST /api/system/reload-plugins`，确认已成功加载
7. 通过 `GET /api/plugins/<name>/contract` 再看一次运行时契约结果
8. 在当前实现下，先在插件管理里完成配置保存与测试连接
9. 到临时邮箱页面实际创建邮箱并读信
10. 再走 external temp-mail 链路做一次任务侧验收
...
8. 能读取消息列表
9. 能读取单封详情
10. 能删除单封 / 清空邮箱 / 删除邮箱；加载失败时 UI 会真实显示错误卡片
...
- `docs/temp-mail-provider-plugin-prompt.md`
