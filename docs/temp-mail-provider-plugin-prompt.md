...
1. `docs/temp-mail-provider-plugin-guide.md`
2. `docs/provider-onboarding.md`
3. `docs/external-integration-quickstart.md`
4. `.trellis/spec/backend/provider-selection-contract.md`（若本地有 Trellis 规范）
...
1. **不要重做插件系统主干**
   ...
   - 不要改产品路径 `/api/temp-emails/*` 与 `/api/v1/external/temp-emails/*`
2. **优先做插件本身**
   - 新增一个 Provider 插件文件
   - 只在确有必要时补少量配套代码
3. **遵守当前真实路径口径**
...
- `docs/temp-mail-provider-plugin-guide.md`
