...
[中文文档](./registration-mail-pool-api.zh.md) | [English Version](./registration-mail-pool-api.en.md)

## Overview

This document describes the `/api/v1/external/*` endpoints currently implemented and exposed for registration workers, scripts, and third-party integrations.
...
- the current version uses `/api/v1/external/*`
- the old anonymous `/api/pool/*` endpoints have been removed
- real registration flows usually need more than `/api/v1/external/pool/*`; they also use the verification and mail-reading endpoints

---

## Authentication and Access Rules

All `/api/v1/external/*` endpoints require:
...
- `/api/v1/external/pool/*` requires both:
...
| --- | --- | --- |
| `GET /api/v1/external/health` | service health check | Yes |
| `GET /api/v1/external/capabilities` | inspect currently available capabilities | Yes |
| `GET /api/v1/external/providers/{kind}/{provider}/health` | inspect provider readiness and optionally probe upstream | Optional |
| `GET /api/v1/external/account-status` | inspect account existence and readability | Optional |

### Mail Reading and Verification

| Endpoint | Purpose | Recommended |
| --- | --- | --- |
| `GET /api/v1/external/messages` | list message summaries | Optional |
| `GET /api/v1/external/messages/latest` | get the latest matching message | Yes |
| `GET /api/v1/external/messages/{message_id}` | get message details | Optional |
| `GET /api/v1/external/messages/{message_id}/raw` | get raw message content | Optional for debugging |
| `GET /api/v1/external/verification-code` | extract a verification code | Common |
| `GET /api/v1/external/verification-link` | extract a verification link | Common |
| `GET /api/v1/external/wait-message` | wait for a new message | Common |
| `GET /api/v1/external/probe/{probe_id}` | query async wait status | Needed for `mode=async` |

### Mail Pool

| Endpoint | Purpose | Recommended |
| --- | --- | --- |
| `POST /api/v1/external/pool/claim-random` | claim a mailbox | Common |
| `POST /api/v1/external/pool/claim-release` | release a mailbox | Common |
| `POST /api/v1/external/pool/claim-complete` | submit the task result | Common |
| `GET /api/v1/external/pool/stats` | inspect pool counts | Optional |
...
Purpose: inspect currently available capabilities and discover the machine-readable contract for provider discovery, mail reading, pool lifecycle actions, and task temp-mail lifecycle actions.
...
The `features` list contains only capabilities that the current API key can use now. `restricted_features` reports disabled or permission-gated capabilities, including public-mode restrictions and multi-key pool access restrictions.

`defaults.pool_claim_provider` reports the provider used when `claim-random` omits `provider`; `defaults.pool_claim_provider_env` is the deployment override env var name, currently `EXTERNAL_POOL_DEFAULT_PROVIDER`. `auto` means request-driven automatic claiming. `deployment_env` summarizes the three deployment-level provider env vars: `ACTIVE_MAILBOX_PROVIDERS`, `EXTERNAL_POOL_DEFAULT_PROVIDER`, and `TEMP_MAIL_PROVIDER`. `OUTLOOK_EMAIL_PROVIDER_CONFIG_FILE` can point to a JSON/TOML provider-selection file with `temp_mail_provider`, `pool_default_provider`, and `active_mailbox_providers`; environment variables still take priority over this file. `deployment_profile` aggregates provider values, required/optional/secret environment variables, settings keys, aliases, per-provider activation examples, a ready-to-write `.env` template, and JSON/TOML templates for provider selection only. `provider_filter` reports whether the deployment is restricted to an allowlist and uses `ACTIVE_MAILBOX_PROVIDERS` as the deployment override env var. `provider_diagnostics` is a local readiness summary with enabled, ready, needs-config counts and diagnostic scope; it only reads local settings, environment variables, provider config files, and the allowlist, and does not probe upstream networks. Use `GET /api/v1/external/providers/{kind}/{provider}/health?probe_network=true` when a caller explicitly wants a non-destructive upstream probe; without `probe_network=true`, it returns local readiness only.
...
`external_mailbox_read_contract.read_by` currently includes both `email` and `claim_token`. Pool responses also expose a pool-specific read contract with `release_claim` and `complete_claim` next actions. Task temp-mail responses expose a task-specific read contract with the `finish_task_mailbox` next action.

### `GET /api/v1/external/providers/{kind}/{provider}/health`

Purpose: inspect one provider's local readiness and, when `probe_network=true` is explicitly supplied, run a non-destructive upstream probe for temp-mail providers.
...
By default this endpoint does not call upstream providers; it only reads local config and allowlist state. Account providers do not run upstream login probes here. If a provider is unconfigured or inactive, `probe_network=true` returns `probe.status=skipped`. `probe.details` filters sensitive fields such as token, key, secret, password, bearer, jwt, and authorization.
...
- if the mailbox came from the pool, use either the `email` or the `claim_token` returned by `claim-random`
- `claim_token` reading automatically uses the claim time as the baseline, so callers avoid messages that arrived before the mailbox was claimed
...
| `provider` | string | No | provider filter: account provider, temp-mail provider, or `auto` |

Current implementation notes:

- the current pool API supports filtering only by `provider`
- when `provider` is omitted, the service uses the configured default from `EXTERNAL_POOL_DEFAULT_PROVIDER` or `pool_default_provider`; `auto` or an empty default keeps automatic claiming
- `ACTIVE_MAILBOX_PROVIDERS` or `active_mailbox_providers` can restrict provider discovery, `auto` claiming, explicit provider claiming, and task temp-mail apply to selected providers
...
- `/api/v1/external/pool/stats` still aggregates only `accounts.pool_status`; there is no separate project-success counter in the response
...
1. move old `/api/pool/*` calls to `/api/v1/external/pool/*`
