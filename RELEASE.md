...
2. **CI 全绿**
   ...
   - `Code Quality / Repository Readiness` 必须通过；它会运行 `python scripts/project_readiness_check.py`，检查外部接入文档、Provider onboarding、env/config 示例、starter clients、live smoke checker 和明显密钥泄露
   - 注意：`Code Quality` / `Python Tests` / `Build and Push Docker Image` 都带 `paths` 过滤，纯文档提交不会自动重跑；发版时必须检查“版本提交 / tag 对应的那次运行”，不能拿后续 docs-only push 的结果替代
