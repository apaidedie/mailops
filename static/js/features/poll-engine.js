        ...
        // Soft-load bridge: seed standard-view emailListCache from poll folder payloads.
        var POLL_LIST_FOLDERS = ['inbox', 'sentitems'];

        function seedPollEmailListCache(email, folder, payload) {
            if (!email || !folder || !payload) return;
            if (typeof cacheBatchFetchedFolder === 'function') {
                cacheBatchFetchedFolder(email, folder, payload);
            } else if (typeof emailListCache !== 'undefined' && emailListCache) {
                var cacheKey = email + '_' + folder;
            ...
            // Drop warm detail for this mailbox+folder so soft re-open refetches after list refresh.
            if (typeof window !== 'undefined' && typeof window.clearEmailDetailCacheForMailbox === 'function') {
                window.clearEmailDetailCacheForMailbox(email, folder);
            }
        }
            ...
            Promise.allSettled([
                ...
                Promise.all(results.map(function(r) {
                    ...
                    dataArray.forEach(function(data, index) {
                        ...
                        // Keep list soft-cache warm for standard mailbox re-open.
                        seedPollEmailListCache(email, POLL_LIST_FOLDERS[index] || 'inbox', data);
                    });
