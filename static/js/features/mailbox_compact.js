        ...
        function formatCompactSelectedCount(count) {
            ...
            return translateCompactText('已选 ' + count + ' 项');
        }

        function formatCompactAccountCount(count) {
            const safeCount = Number(count || 0);
            return translateCompactText(safeCount + ' 个账号');
        }
        ...
        function switchMailboxViewMode(mode) {
            ...
            mailboxViewMode = ['standard', 'compact', 'unified'].includes(mode) ? mode : 'unified';
            ...
            const unifiedLayout = document.getElementById('mailboxUnifiedLayout');
            ...
            if (unifiedLayout) {
                unifiedLayout.style.display = mailboxViewMode === 'unified' ? 'block' : 'none';
            }
            ...
            if (mailboxViewMode === 'unified') {
                ...
                if (typeof bindUnifiedMailboxControls === 'function') {
        ...
        function renderCompactGroupStrip(groupItems, activeGroupId) {
            // Compact strip is mailbox compact-mode chrome only. Soft loadGroups /
            // language change off that surface must not rewrite hidden DOM.
            if (typeof currentPage !== 'undefined' && currentPage !== 'mailbox') {
                return;
            }
            if (typeof mailboxViewMode !== 'undefined' && mailboxViewMode !== 'compact') {
                return;
            }
        ...
        async function copyCompactVerification(account, buttonElement) {
            ...
            if (account.latest_verification_code) {
                try {
                    ...
                    showToast(
                        translateCompactText('已复制: ' + account.latest_verification_code),
                        'success'
                    );
                    return;
                } catch (error) {
        ...
        async function refreshCompactAccount(accountId, buttonElement) {
            ...
            try {
                // Soft-load bridge: seed emailListCache so standard-view re-select keeps compact pull pages.
                const folderSpecs = [
                    { folder: 'inbox', request: fetch(`/api/emails/${encodeURIComponent(account.email)}?folder=inbox&skip=0&top=10`) },
                    { folder: 'junkemail', request: fetch(`/api/emails/${encodeURIComponent(account.email)}?folder=junkemail&skip=0&top=10`) }
                ];
                const results = await Promise.allSettled(folderSpecs.map(spec => spec.request));
                let hasSuccess = false;
                for (let i = 0; i < results.length; i++) {
                    const result = results[i];
                    const folder = folderSpecs[i].folder;
                    ...
                    // Upsert list cache for standard-view soft-load (same shape as batch fetch).
                    if (typeof cacheBatchFetchedFolder === 'function') {
                        cacheBatchFetchedFolder(account.email, folder, payload);
                    } else if (typeof emailListCache !== 'undefined' && emailListCache) {
                        const cacheKey = `${account.email}_${folder}`;
                        emailListCache[cacheKey] = {
                            emails: (typeof sortEmailsByNewestFirst === 'function')
                                ? sortEmailsByNewestFirst(payload.emails || [])
                                : (payload.emails || []),
                            has_more: payload.has_more || false,
                            skip: 0,
                            method: payload.method || 'Graph API'
                        };
                    }
                    // Force detail soft-load to refetch for this mailbox+folder after list refresh.
                    if (typeof window.clearEmailDetailCacheForMailbox === 'function') {
                        window.clearEmailDetailCacheForMailbox(account.email, folder);
                    }
                }
                if (!hasSuccess) {
        ...
        function renderCompactAccountList(accounts) {
            // Compact inventory chrome only while mailbox compact mode is active.
            if (typeof currentPage !== 'undefined' && currentPage !== 'mailbox') {
                return;
            }
            if (typeof mailboxViewMode !== 'undefined' && mailboxViewMode !== 'compact') {
        ...
        window.addEventListener('ui-language-changed', () => {
            // Compact chrome only while on mailbox compact mode (helpers no-op otherwise).
            renderCompactGroupStrip(groups, currentGroupId);
            if (
                (typeof currentPage === 'undefined' || currentPage === 'mailbox')
                && (typeof mailboxViewMode === 'undefined' || mailboxViewMode === 'compact')
            ) {
                renderCompactAccountList(getCompactVisibleAccounts());
            }
        });
