(function () {
    ...
    const exactMap = {
        ...
        '暂无已安装插件': 'No plugins installed',
        '内置仅 Cloudflare / GPTMail。Mail.tm、DuckMail 等请点「自定义安装」上传 Provider 插件（.py），安装后会出现在上方 Provider 列表。': 'Built-ins are Cloudflare and GPTMail only. Use Install for official Mail.tm / DuckMail plugins, or Custom install for your own .py plugin.',
        '修改 Provider / API Key / 域名后请保存': 'Save after changing provider, API key, or domains',
        '内置：Cloudflare、GPTMail。Mail.tm / DuckMail / TempMail.lol / Emailnator 请在下方插件管理中点击「安装」后使用。': 'Built-in: Cloudflare and GPTMail. Install Mail.tm / DuckMail / TempMail.lol / Emailnator from plugin management below.',
