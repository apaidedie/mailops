(function () {
    ...
    const exactMap = {
        ...
        '暂无已安装插件': 'No plugins installed',
        '内置仅 Cloudflare Temp Mail。GPTMail / Mail.tm / DuckMail / TempMail.lol / Emailnator 请在下方插件管理中安装后使用。': 'Built-in is Cloudflare Temp Mail only. Install GPTMail / Mail.tm / DuckMail / TempMail.lol / Emailnator from plugin management below.',
        '内置仅 Cloudflare Temp Mail。GPTMail / Mail.tm / DuckMail / TempMail.lol / Emailnator 可在列表中点「安装」（官方捆绑包，无需外网）；也可「自定义安装」上传自己的 .py。': 'Built-in is Cloudflare Temp Mail only. Install GPTMail / Mail.tm / DuckMail / TempMail.lol / Emailnator from the list (bundled, offline), or Custom install your own .py.',
        '修改 Provider / API Key / 域名后请保存': 'Save after changing provider, API key, or domains',
        'GPTMail 用量': 'GPTMail usage',
        '今日已用': 'Used today',
        '今日限额': 'Daily limit',
        '今日剩余': 'Remaining today',
        '累计用量': 'Total usage',
        '累计限额': 'Total limit',
        '累计剩余': 'Remaining total',
        '刷新用量': 'Refresh usage',
        '用量已更新': 'Usage updated',
        '已从 GPTMail /api/stats 刷新用量': 'Usage refreshed from GPTMail /api/stats',
        '配置 GPTMail API Key 后可查看用量': 'Configure the GPTMail API key to view usage',
        '点击「刷新用量」从 GPTMail /api/stats 拉取限额': 'Click Refresh usage to load quotas from GPTMail /api/stats',
        '上游未返回 usage 字段': 'Upstream did not return a usage object',
        '未获取到用量': 'Usage not available',
        '探测完成，但上游返回异常': 'Probe finished, but upstream returned an error',
