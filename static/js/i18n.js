(function () {
    ...
    const exactMap = {
        ...
        '内置仅 Cloudflare / GPTMail。Mail.tm、DuckMail 等请点「自定义安装」上传 Provider 插件（.py），安装后会出现在上方 Provider 列表。': 'Built-ins are Cloudflare and GPTMail only. Install Mail.tm / DuckMail etc. via Custom install (.py plugins); they then appear in the provider list above.',
        '修改 Provider / API Key / 域名后请保存': 'Save after changing provider, API key, or domains',
        '内置仅 Cloudflare；GPTMail 为官方内置可选。Mail.tm / DuckMail 等请通过下方插件管理安装后使用。': 'Built-in: Cloudflare; GPTMail is an official optional built-in. Install Mail.tm / DuckMail etc. via plugin management below.',
