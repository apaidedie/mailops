...
class CloudflareTempMailProvider(TempMailProviderBase):
    ...
    def health_check(self) -> dict[str, Any]:
        result = self.get_cf_worker_domains()
        if not result.get("success"):
            return {
                "success": False,
                "method": "open_api_settings",
                "error_code": result.get("error_code") or "UPSTREAM_SERVER_ERROR",
                "error": result.get("error") or "CF Worker settings probe failed",
        ...
        return {
            ...
            "details": {
                ...
                "default_domain": result.get("default_domain") or "",
