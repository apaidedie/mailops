...
- 校验镜像名白名单（仅允许 ghcr.io/apaidedie/mailops）
- 操作前记录审计日志
...
def _looks_like_local_image_ref(image_ref: str) -> bool:
    ...
    # 无 namespace（如 mailops:latest）通常是本地构建
    # 官方镜像应包含 namespace：ghcr.io/apaidedie/mailops
    if "/" not in ref:
        return True
...
def validate_image_for_update(image_ref: str, *, image_id: Optional[str] = None) -> Tuple[bool, str]:
    ...
    if not ok:
        ...
        if _looks_like_local_image_ref(image_ref):
            return (
                False,
                f"检测到本地构建/非官方镜像（{image_ref}），已按安全策略禁止 Docker API 一键更新。请使用官方远程镜像部署（如 ghcr.io/apaidedie/mailops:latest）。",
            )
        return ok, msg
...
def validate_image_name(image_name: str) -> Tuple[bool, str]:
    ...
    Args:
        image_name: 镜像名（如 ghcr.io/apaidedie/mailops:latest）
...
def get_current_container_info() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        client = docker.from_env()
        containers = client.containers.list(filters={"name": "mailops"})
        if not containers:
...
def spawn_update_helper_container(
    ...
    *,
    ...
    try:
        ...
        helper_cmd = ["python", "-m", "mailops.services.docker_update_helper"]
