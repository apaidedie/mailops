...
from mailops.services.temp_mail_provider_base import TempMailProviderBase, normalize_provider_capabilities
from mailops.services.temp_mail_provider_contract import sanitize_config_schema, validate_temp_mail_provider_class
...
_BUILTIN_PROVIDERS = {
    ...
    settings_repo.MAIL_TM_TEMP_MAIL_PROVIDER,
    settings_repo.TEMPMAIL_LOL_TEMP_MAIL_PROVIDER,
    settings_repo.EMAILNATOR_TEMP_MAIL_PROVIDER,
    settings_repo.DUCKMAIL_TEMP_MAIL_PROVIDER,
}

# 触发内置 provider 装饰器注册
import mailops.services.temp_mail_provider_cf  # noqa: F401,E402
import mailops.services.temp_mail_provider_custom  # noqa: F401,E402
import mailops.services.temp_mail_provider_public  # noqa: F401,E402
...
def get_temp_mail_provider(provider_name: str | None = None) -> TempMailProviderBase:
    ...
    from mailops.services.provider_catalog import is_mailbox_provider_active

    if not is_mailbox_provider_active("temp", resolved_provider_name):
        raise TempMailProviderFactoryError(
...
def get_available_providers() -> list[dict[str, Any]]:
    ...
    for name, cls in sorted(registry.items(), key=lambda item: item[0]):
        raw_config_schema = getattr(cls, "config_schema", {})
        config_schema = sanitize_config_schema(copy.deepcopy(raw_config_schema)) if isinstance(raw_config_schema, dict) else {}
        capabilities = normalize_provider_capabilities(getattr(cls, "provider_capabilities", None))
        contract_validation = validate_temp_mail_provider_class(name, cls, probe_options=False)
        providers.append(
            {
                "name": name,
