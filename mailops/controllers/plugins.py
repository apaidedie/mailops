...
from mailops.services.temp_mail_plugin_manager import (
    ...
    get_plugin_contract_validation,
    install_plugin,
    read_plugin_config,
    save_plugin_config,
...
from mailops.services.temp_mail_provider_contract import contract_validation_summary
from mailops.services.temp_mail_provider_factory import get_plugin_load_state
...
def api_get_plugins():
    ...
    try:
        ...
        contract_state = {
            str(item.get("name") or ""): contract_validation_summary(item.get("contract_validation"))
    ...
    for plugin_name in all_names:
        ...
        if plugin_name in contract_state:
            plugin["contract_validation"] = contract_state[plugin_name]
...
def api_get_plugin_contract(name: str):
    try:
        return _ok(get_plugin_contract_validation(name))
    except PluginManagerError as err:
        return _error(err)
