...
def insert_claimed_temp_mailbox(
    ...
    *,
    ...
    source: str = temp_emails_repo.DEFAULT_TEMP_MAIL_SOURCE,
    meta: Optional[dict] = None,
    visible_in_ui: bool = False,
) -> dict:
    """插入一个 temp_emails 记录并直接标记为 claimed。"""
    ...
    if "@" in normalized_email:
        email_prefix, email_domain = normalized_email.rsplit("@", 1)
        normalized_prefix = normalized_prefix or email_prefix.strip()
        normalized_domain = normalized_domain or email_domain.strip().lower()

    normalized_provider = str(provider_name or "").strip()
    if not normalized_provider:
        raise PoolRepositoryError("provider_name 不能为空", "invalid_provider")

    meta_obj = dict(meta or {})
    meta_obj["provider_name"] = normalized_provider
    meta_json = temp_emails_repo.serialize_temp_email_meta(meta_obj, source=source)
    ...
    try:
        ...
        logger.info(
            "[pool] 动态插入临时邮箱并 claim: %s (provider=%s, temp_id=%s, account_id=%s)",
            normalized_email,
            normalized_provider,
            temp_id,
            account_id_from_temp_id(temp_id),
        )

        return {
            "id": account_id_from_temp_id(temp_id),
            "email": normalized_email,
            "email_domain": normalized_domain or "",
        ...
        try:
            ...
            pass
        raise PoolRepositoryError(f"插入临时邮箱失败: {e}", "db_integrity_error") from e
    except Exception as e:
        try:
            conn.execute("ROLLBACK")
        except Exception:
            pass
        raise PoolRepositoryError(f"插入临时邮箱失败: {e}", "db_error") from e
...
def claim_atomic(
    ...
    elif normalized_provider_names:
        placeholders = ",".join(["?"] * len(normalized_provider_names))
        sql += f" AND lower(a.provider) IN ({placeholders})"
        params.extend(sorted(normalized_provider_names))
...
def claim_temp_mailbox_atomic(
    ...
    *,
    ...
    conn.execute("BEGIN IMMEDIATE")
    if normalized_provider_names:
        mailbox = None
        for candidate in conn.execute(sql, params).fetchall():
            if _temp_mailbox_provider_name(candidate).lower() in normalized_provider_names:
                mailbox = candidate
                break
    else:
    ...
    logger.info(
        "[pool] 领取临时邮箱: %s (provider=%s, temp_id=%s, account_id=%s)",
        email_addr,
        provider_name,
        mailbox["id"],
