...
def create_app(*, autostart_scheduler: Optional[bool] = None):
    ...
    if _APP_INSTANCE is None:
        ...
        from mailops import config as app_config
        from mailops.db import init_db, register_db
        from mailops.middleware import (
        ...
        from mailops.security.csrf import init_csrf
        ...
        # 统一配置 mailops / Flask 日志；默认文本格式，生产可切换为逐行 JSON。
        from mailops.logging_config import configure_runtime_logging
        ...
        app.after_request(attach_security_headers)
        ...
        app.register_blueprint(mailboxes.create_blueprint())
        ...
        # 外部 API 浏览器来源策略；默认保留扩展来源，普通 Web 来源必须显式配置。
