...
- Many tests import symbols from `outlook_web.controllers.accounts`, `outlook_web.services.external_api`, and `outlook_web.db`.
...
1. **Backend package splits (P0):** Convert fat modules into packages with thin public re-exports where useful:
   ...
   - `db` → package preserving import path `outlook_web.db` for `get_db`, `init_db`, `create_sqlite_connection`, schema helpers
...
- [ ] `from outlook_web.db import get_db` (and other currently public db helpers used in-repo) still works.
