...
The central backend boundary is `mailops.services.provider_catalog`. It already builds `deployment_profile`, `selection_policy`, `provider_integration_guide`, `integration_manifest`, and mailbox `provider_context`. The new `documentation` object should be produced from one helper so these payloads stay aligned.

The OpenAPI boundary is `mailops.services.external_api_openapi`. Schemas need to tolerate the new object and describe it for external client generation.
