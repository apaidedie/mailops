...
3. Implement `LOG_FORMAT`, `LOG_LEVEL`, and backward-compatible `PERF_LOGGING` helpers in `outlook_web/config.py`.
4. Add `outlook_web/logging_config.py` with managed handler setup, request context filter, text formatter, and JSON formatter.
5. Replace inline logging setup in `outlook_web/app.py` with the shared configurator.
...
`black --check outlook_web/logging_config.py outlook_web/config.py outlook_web/app.py tests/test_logging_config.py tests/test_project_readiness_check.py`

`isort --check-only --diff --profile black outlook_web/logging_config.py outlook_web/config.py outlook_web/app.py tests/test_logging_config.py tests/test_project_readiness_check.py`

`flake8 outlook_web/logging_config.py outlook_web/config.py outlook_web/app.py tests/test_logging_config.py tests/test_project_readiness_check.py --count --select=E9,F63,F7,F82 --show-source --statistics`
